...
| Maps / Monde | Métadonnées par map (nombre de vagues, multiplicateur de difficulté) portées par un **bloc de configuration** posé dans la map, avec son écran comme le spawner. | Moyen | Idée | Écarté pour l'instant (2026-09-02) au profit de champs sur l'enum `GameMap` : ajouter une map demande **déjà** une modification de code (entrée d'enum, clé de lang, image d'aperçu), donc un bloc de config n'apporterait aucune autonomie au mappeur — il déplacerait 3 nombres hors d'un fichier déjà édité, contre un bloc + menu + écran + paquet + persistance + le code pour le retrouver dans la structure posée. À reconsidérer le jour où une map s'ajoute sans toucher au code (définitions en JSON, aperçu chargé par convention de nom). |
