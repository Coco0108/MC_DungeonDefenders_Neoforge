...
**Classes / Héros**, **Sorts**, **Armes / Armures**, **Score / Progression**,
**Modèles / Textures**, **Divers**.
...
| Maps / Monde | Zones **interdites** à la pose de tours, marquées par le créateur de map (liste noire, pas liste blanche). | Moyen | Faite | Fait le 2026-09-02 : `NoBuildZoneBlock`, un marqueur invisible/traversable par case interdite, qui bloque la pose en occupant simplement la case (`canBeReplaced()` est déjà testé côté client et serveur). Option "marqueur de volume" écartée au profit d'un bloc par case + `/fill`. Voir [02-gameplay.md](02-gameplay.md#les-zones-interdites-à-la-pose--blocknobuildzoneblockjava). |
| HUD / UI | Surbrillance des zones interdites pendant le mode pose : voir d'un coup d'œil les cases où l'on ne peut pas construire, au lieu de découvrir le refus case par case. | Moyen | Idée | Mis de côté par le joueur (2026-09-02) au moment d'ajouter `NoBuildZoneBlock` — à reconsidérer quand une vraie map en contiendra beaucoup et que les repérer une par une deviendra pénible. Techniquement proche de l'hologramme de pose déjà en place (`TowerPlacementClientEvents`, submit-node pipeline) : il faudrait un balayage des marqueurs autour du joueur pendant le mode pose, et un rendu de surface plutôt qu'un contour. |
| Maps / Monde | Aperçu de map capturé en jeu (bouton "prendre l'aperçu" + capture d'écran recadrée en 128x72, stockée dans la sauvegarde et diffusée aux clients). | Moyen | Prête | Phase 2 du chantier "maps en jeu" (2026-09-02). Déroulé validé avec le joueur : le bouton arme la capture, ferme l'écran, le joueur cadre, une touche déclenche. `Screenshot.takeScreenshot(...)` rend une `NativeImage` du buffer de rendu ; le HUD doit être masqué le temps d'une frame. À 128x72 la PNG pèse 5-15 Ko, donc aucun souci de réseau. |
| Maps / Monde | Commande `/dd_export <map>` produisant un jar d'extension prêt à publier (mods.toml généré + structure + aperçu). | Moyen | Idée | Phase 3 du chantier "maps en jeu". Rend la publication sur CurseForge accessible sans que l'auteur touche à un fichier — c'est la promesse "pas trop compliqué de faire une map". |
| Maps / Monde | Une phase (ou un état) "taverne" distincte de Construction, pour pouvoir construire dans la taverne sans que la roue des tours s'y confonde avec une partie. | Moyen | Faite | Fait le 2026-09-01 : `GamePhase.TAVERN`, pose/suppression de tours autorisées mais gratuites et sans remboursement, spawners et vagues inactifs, HUD Vague/Ennemis masqués. Voir [02-gameplay.md](02-gameplay.md#la-phase-taverne--gamephasetavern). |
| Maps / Monde | Mannequin d'entraînement : cible immobile à PV infinis, attaquée par les tours, pour mesurer leurs dégâts. | Moyen | Faite | Annoncé par le joueur (2026-08-31) pour la taverne. Les tourelles ne ciblent que des `Monster` (`AbstractTurretBlockEntity#findTarget`) : le mannequin devra donc en être un, ou le ciblage devra s'élargir. Fait le 2026-08-31 : `TrainingDummyEntity` (sous-classe de `Zombie`, aucun goal, 1024 PV remis au max à chaque coup) plus `TrainingDummyBlock`, qui la réinvoque si elle a disparu — l'option "bloc" a été retenue, elle resiste à un nettoyage de zone qui raterait l'ancienne. Voir [02-gameplay.md](02-gameplay.md#le-mannequin-dentraînement--entitytrainingdummyentityjava-blocktrainingdummyblockjava). |
| Modèles / Textures | Modèle et texture dédiés pour le **mannequin d'entraînement** — un vrai mannequin de paille, à la place du rendu de zombie vanilla réutilisé aujourd'hui. | Moyen | Idée | Annoncé par le joueur (2026-09-01) : il fera le modèle. Côté code, le changement est petit et localisé — remplacer `ZombieRenderer::new` par un renderer dédié dans `DungeonDefendersModClient#onRegisterRenderers`, plus un `EntityRendererProvider`/modèle et l'enregistrement de sa layer (`EntityRenderersEvent.RegisterLayerDefinitions`). **`TrainingDummyEntity` peut rester une sous-classe de `Zombie`** : c'est imposé par le ciblage des tourelles (`Monster`), pas par le rendu. Livraison des fichiers selon le workflow habituel : `.bbmodel` + PNG dans `art/models/training_dummy/`. Voir [02-gameplay.md](02-gameplay.md#le-mannequin-dentraînement--entitytrainingdummyentityjava-blocktrainingdummyblockjava). |
