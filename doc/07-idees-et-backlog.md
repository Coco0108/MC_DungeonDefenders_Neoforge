...
| Maps / Monde | Mannequin d'entraînement : cible immobile à PV infinis, attaquée par les tours, pour mesurer leurs dégâts. | Moyen | Prête | Annoncé par le joueur (2026-08-31) pour la taverne. Les tourelles ne ciblent que des `Monster` (`AbstractTurretBlockEntity#findTarget`) : le mannequin devra donc en être un, ou le ciblage devra s'élargir. Reste à trancher : entité posée dans le `.nbt` de la taverne, ou entité invoquée/entretenue par un bloc dédié (patron `SpawnerBlock`) — la seconde option resiste mieux à un nettoyage de zone qui raterait l'ancienne. Le nettoyage d'entités nécessaire est déjà fait (`TavernSpawn#clearZone`). |
