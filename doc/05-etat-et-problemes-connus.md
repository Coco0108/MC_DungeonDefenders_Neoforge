...
- ✅ Reste du roster de tours de l'Écuyer (2026-08-29, noms repris du plan Excel du joueur) :
  ...
  - Cette branche est partie de `main`, en amont des deux corrections déjà décrites ci-dessus
    pour Harpoon Turret (cooldown/flèche figée) : en reconstruisant `AbstractTurretBlockEntity`
    pour ce roster, ces deux mêmes correctifs ont été réappliqués (même valeur `-attackIntervalTicks`,
    même décalage de 0,6 bloc, centralisé dans le nouveau `muzzlePosition`) — pas une régression
